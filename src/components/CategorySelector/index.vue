<template>
  <el-card>
    <el-form :inline="true" :model="categoryIdForm" class="demo-form-inline">
      <el-form-item label="一级分类">
        <el-select v-model="categoryIdForm.category1Id" placeholder="选择一级类别" @change="touchC2" :disabled="isDisabled">
          <el-option
      ...
      <el-form-item label="二级类别">
        <el-select v-model="categoryIdForm.category2Id" placeholder="选择二级类别" @change="touchC3" :disabled="isDisabled">
          <el-option
      ...
      <el-form-item label="三级类别">
        <el-select v-model="categoryIdForm.category3Id" placeholder="选择三级类别" @change="touchOver" :disabled="isDisabled">
          <el-option
  ...
  export default {
    ...
    methods: {
      ...
      async touchC2(category1Id) {
        ...
        // 向父组件传送一级类别Id
        this.$emit('handleAttr', {categoryId: category1Id, level: 1})
      ...
      async touchC3(category2Id) {
        ...
        // 向父组件发送二级类别Id
        this.$emit('handleAttr', {categoryId: category2Id, level: 2})
      ...
      touchOver(category3Id) {
        this.$emit('handleAttr', {categoryId: category3Id, level: 3})
      }
    }
  }
</script>
